...
function processRegEx(editor, runAllAbove = false) {
    const config = vscode.workspace.getConfiguration('pythonREPL.cell');
...
async function getCurrentPythonInterpreter() {
    try {
...
async function detectAndActivateEnvironment() {
    ...
    if (interpreterPath) {
        ...
        if (interpreterPath.includes('conda') || interpreterPath.includes('envs')) {
            ...
            const envIndex = pathParts.findIndex((part) => part === 'envs');
...
async function saveFileBeforeSend(editor) {
    const config = vscode.workspace.getConfiguration('pythonREPL');
...
async function prepareForExecution(editor) {
    if (editor) {
...
function activate(context) {
    ...
    ;
    function sendLines(startLine, endLine, checkCwd = false) {
        const config = vscode.workspace.getConfiguration('pythonREPL');
        const directSend = config.get('sendTextDirectly');
